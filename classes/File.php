...
class File {
	...
	public function upload($file, $allowed_ext, $max_filesize = 20971520) {
		...
		if (!move_uploaded_file($filetmpname, 'media/files/' . $uploaded_filename)) {
	...
	public function image_resize($width, $height, $filename) {
		...
		list($w, $h) = getimagesize('media/files/' . $filename);
		...
		$imgString = file_get_contents('media/files/' . $filename);
