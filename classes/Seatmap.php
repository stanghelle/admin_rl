...
//		}
	...
	public function outputSeats($from, $to, $type, $direction) {
		...
		for ($i = $from; $i <= $to; $i++) {
			...
			$seat_status = $occupied_by_id;
				if ($seat_status == 0) {
					echo "<td class='seat seat-other' onclick='viewSeat({$i});' data-toggle='tooltip' data-placement='top' data-container='body' title='Ledig!'>{$i}</td>";
				} else {
					$userObj = new User($occupied_by_id);
					$occupied_by_name = $userObj->data()->name;
					echo "<td class='seat seat-other' onclick='viewSeat({$i});' data-toggle='tooltip' data-placement='top' data-container='body' title='{$occupied_by_name} er Ankommet'>{$i}</td>";
				}
