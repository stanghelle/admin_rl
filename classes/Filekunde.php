...
class Filekunde {
	...
	public function upload($file, $allowed_ext, $max_filesize = 20971520) {
		...
		if (!move_uploaded_file($filetmpname, '../images/kunde/' . $uploaded_filename)) {
	...
	public function image_resize($width, $height, $filename) {
		...
		list($w, $h) = getimagesize('../images/kunde/' . $filename);
		...
		$imgString = file_get_contents('../images/kunde/' . $filename);
