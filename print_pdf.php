<?php
/**
 * Print PDF - Generate program schedule PDF
...
// Get week and year info from dagid=8
$weekData = $db->query("SELECT kl, program FROM prg_pdf WHERE dagid = ? ORDER BY sort_order ASC, id ASC LIMIT 1", [8]);

$uke = date('W'); // Default to current week
$aar = date('Y'); // Default to current year
 ...
 */
function generateDayContent($dagid, $isAndakt = false) {
    $contents = '';
    $db = DB::getInstance();

    $data = $db->query(
        "SELECT kl, program FROM prg_pdf WHERE dagid = ? ORDER BY sort_order ASC, id ASC",
        [$dagid]
    );
...
$row1 = generateDayContent(1);
$row2 = generateDayContent(2);
$row3 = generateDayContent(3);
$row4 = generateDayContent(4);
$row5 = generateDayContent(5);
$row6 = generateDayContent(6);
$row7 = generateDayContent(7);
$row9 = generateDayContent(9, true); // Andakt
...
$pdf->SetTitle("Programoversikt Radio Lyngdal");
...
$pdf->Output('Programoversikt uke' . $uke . '.pdf', 'D');
?>
